...
js-target = scripts/build/hangoverx.js
js-target-min = scripts/build/hangoverx.min.js
...
# to use `make deploy` to deploy HangoverX to your own server, create
...
deploy-debug: debug
	...
	@mkdir _site
	@cp -R levels scripts styles images sound index.html _site
	@./deploy.sh /hangoverx/debug _site
	@rm -rf _site
	@echo "[ Done ]"
...
deploy-github:
	@git checkout gh-pages && git rebase master && make release && git commit -am build && git push -uf origin gh-pages && git checkout master && make
